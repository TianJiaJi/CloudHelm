...
import { Activity, AlertTriangle, Bot, CheckCircle2, ChevronRight, CloudCog, Gauge, Link2, Loader, LockKeyhole, RefreshCw, Rocket, Settings2, ShieldAlert, Terminal, Zap } from 'lucide-vue-next'
...
const api = async (path, options = {}) => {
  ...
  try {
    const response = await fetch(path, { headers: { 'Content-Type': 'application/json' }, ...options, signal: controller.signal })
    const text = await response.text()
    let data = null
    try { data = JSON.parse(text) } catch { /* non-JSON body (e.g. proxy error page) */ }
    if (!response.ok) throw new Error(data?.detail || data?.message || `请求失败（HTTP ${response.status}）`)
    if (data === null) throw new Error('后端返回了非 JSON 响应，请检查服务是否正常')
    if (data.mode) lastMode = data.mode
    return data
  } catch (error) {
    // A hung backend must not freeze the console forever in "执行中".
    if (error.name === 'AbortError') throw new Error(`请求超时（${REQUEST_TIMEOUT_MS / 1000} 秒无响应），已自动取消`)
    throw error
  } finally {
    clearTimeout(timer)
  }
...
// Three states per spec: green=healthy, yellow=starting, red=faulted.
const DOWN_STATES = ['Terminating', 'Failed', 'Unknown', 'Error']
const podState = (pod) => pod.ready ? 'up' : (DOWN_STATES.includes(pod.status) ? 'down' : 'starting')
const POD_ICON = { up: CheckCircle2, down: AlertTriangle, starting: Loader }
...
<template>
  <main class="shell">
    ...
    <section class="main-grid"><div class="panel chart-panel"><div class="panel-heading"><div><p class="eyebrow">TRAFFIC OVERVIEW</p><h2>流量趋势</h2></div><span class="live-pill"><i></i> 每 2 秒更新</span></div><div ref="chart" class="chart"></div></div><div class="panel pods-panel"><div class="panel-heading"><div><p class="eyebrow">SERVICE HEALTH</p><h2>服务状态</h2></div><span class="pod-legend"><i class="lg up"></i>健康 <i class="lg starting"></i>启动中 <i class="lg down"></i>故障</span></div><div class="pod-list"><div v-for="pod in pods" :key="pod.name" class="pod-row" :class="podState(pod)"><div class="pod-icon"><component :is="POD_ICON[podState(pod)]" :size="17" /></div><div class="pod-info"><strong>{{ pod.deployment }}</strong><span>{{ pod.name }}</span></div><span class="pod-status">{{ pod.status }}</span></div></div></div></section>
    ...
    <section class="lower-grid"><div class="panel assistant"><div class="panel-heading"><div><p class="eyebrow">AI OPERATIONS AGENT</p><h2><Bot :size="20" /> 运维助手</h2></div><span class="agent-badge">安全模式</span></div><div class="quick-asks"><button @click="ask('请分析当前系统健康度')" :disabled="busy === 'chat'">健康度分析</button><button @click="ask('当前系统有什么瓶颈？')" :disabled="busy === 'chat'">瓶颈分析</button><button @click="ask('帮我排查故障')" :disabled="busy === 'chat'">故障排查</button><button @click="ask('生成一份运维报告')" :disabled="busy === 'chat'">生成报告</button></div><div class="messages"><div v-for="(message, index) in messages" :key="index" :class="['message', message.role]"><span class="message-avatar">{{ message.role === 'ai' ? 'AI' : '我' }}</span><div class="bubble" v-html="renderMarkdown(message.text)"></div><button v-if="message.action" class="suggestion" @click="approve(message.action.action_id)">{{ message.action.label }} · 需要确认</button></div><div v-if="busy === 'chat'" class="message"><span class="message-avatar">AI</span><div class="bubble typing">正在分析…</div></div><div v-if="!messages.length && busy !== 'chat'" class="empty-state">向助手询问系统健康度、瓶颈、故障排查或生成运维报告</div></div><form class="chat-form" @submit.prevent="ask()"><input v-model="question" placeholder="输入运维问题..." :disabled="busy === 'chat'" /><button aria-label="发送" :disabled="busy === 'chat'"><Loader v-if="busy === 'chat'" :size="15" class="spin" /><ChevronRight v-else :size="18" /></button></form></div><div class="panel logs"><div class="panel-heading"><div><p class="eyebrow">EVENT STREAM</p><h2>实时操作日志</h2></div><div class="log-actions"><span class="live-pill"><i></i> LIVE</span><button class="clear-logs" @click="clearLogs">清空日志</button></div></div><div class="log-list"><div v-for="log in logs" :key="log.id" class="log-line"><time>{{ new Date(log.timestamp).toLocaleTimeString('zh-CN', { hour12: false }) }}</time><b :class="log.level.toLowerCase()">{{ log.level }}</b><span>{{ log.message }}</span></div></div></div></section>
  </main>
</template>
