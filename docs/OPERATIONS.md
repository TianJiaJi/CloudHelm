...
| 前端对抗（连点/输入/刷新） | 浏览器实测 | 连点 20 次只部署 1 次；非法副本数全部拦下且不发请求；审批中刷新不再留下悬空请求（TTL 过期） |
| UI 收敛性 | 外部改动服务端状态后观察界面 | ≤1 个轮询周期内自动跟到服务端真相 |
| 轮询稳定性 | 统计 10 秒内 `/api/metrics` 请求数 | 5 次（2 秒间隔），无堆积 |
